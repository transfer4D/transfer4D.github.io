from lepard.lib.trainer import Trainer
import torch
from tqdm import tqdm
from lepard.models.loss import MatchMotionLoss as MML
import numpy as np
from lepard.models.matching import Matching as CM
import math
from lepard.datasets.utils import knn_point_np
...
def blend_anchor_motion (query_loc, reference_loc, reference_flow, query_confidence, knn=3, search_radius=0.1) :
    ...
    corresp = []
    for i,cors in enumerate(idx):
        for j,ind in enumerate(cors):
            if ~mask[i,j]:
                corresp.append([i,ind])
                # print(i,ind,dists[i,j])
    ...
    blended_confidence = np.sum (query_confidence[idx] * weight.reshape ([-1, knn]), axis=1, keepdims=False)

    mask = mask.sum(axis=1)<3

    return blended_flow, corresp,mask,blended_confidence


def blend_anchor_motion_confidence(query_loc, reference_loc, reference_flow, query_confidence, knn=3, search_radius=0.1) :
    '''approximate flow on query points
    this function assume query points are sub- or un-sampled from reference locations
    @param query_loc:[m,3]
    @param reference_loc:[n,3]
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)): # loop through this epoch
                ...
                for k in data:
                    print(k)
                    if type(data[k]) == list:
                        print([x.shape for x in data[k]])
                    elif type(data[k]) == dict:
                        try:
                            print([data[k][x].shape for x in data[k]])
                        except: 
                            continue
                    else:   
                ...
                print(data['coarse_flow'][0])
                print(data['coarse_flow'][0].shape)
