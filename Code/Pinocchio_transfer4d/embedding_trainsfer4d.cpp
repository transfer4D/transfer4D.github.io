...
vector<float> DisplayPenalty(const vector<PenaltyFunction *> &penaltyFunctions,PartialMatch &cur,int next){

    vector<float> penalty_list(penaltyFunctions.size(),0);     
    
    int idx = cur.match.size();
    if(idx == 0)
        return penalty_list; 

    Debugging::out() << "Idx:" << idx << ";" << "Match:" << next << ";"; 
    
    for(int i = 0; i < (int)cur.match.size(); ++i) { 
        Debugging::out() << " " << cur.match[i];
    }    
    Debugging::out() << ";";

    // int next = cur.match[idx-1];
    // cur.match.pop_back();

    double out = 0;
    if(cur.penalty_list.size() == 0) 
        cur.penalty_list.resize(penaltyFunctions.size(),0);
    ...
    for(int i = 0; i < (int)penaltyFunctions.size(); ++i) {
        ...
        if(penalty > 1.)
            out = 2.; 
        else if(penalty < 1. && out < 2.) 
            out += penalty;
        penalty_list[i] = cur.penalty_list[i] + penalty;
        Debugging::out() << penalty_list[i] << " ";
    }
    Debugging::out() << ";New Penalty:" << cur.penalty + out << endl;
                          ...
                          const Skeleton &skeleton, const vector<vector<int> > &possibilities)
{
    ...
    while(!todo.empty()) {
        ...
        if(curSz > maxSz) {
            maxSz = curSz;
            if(maxSz > 3)
                Debugging::out() << "Reached " << todo.size() << " Current Size:" << idx << " out of:" << toMatch << endl;
                
        }

        if(todo.size() > maxQueueSize){
            break;
        }
        ...
        if(idx == toMatch) {
            output = cur;
            Debugging::out() << "Updating output. New Penalty " << cur.penalty << endl;
            Debugging::out() << "Previous idx:" << prevIdx << " New idx:" << idx << endl;
            ...
            if(cur.penalty + extraPenalty < 1.) {
                ...
                for(j = idx + 1; j < toMatch; ++j) {
                    ...
                    for(k = 0; k < (int)possibilities[j].size(); ++k) {
                        // Debugging::out() << "heuristic = ";
                        // DisplayPenalty(penaltyFunctions,next,possibilities[j][k]);
                        minP = min(minP, computePenalty(penaltyFunctions, next, possibilities[j][k], j));
                    }
                    next.heuristic += minP;
                    if(next.heuristic > 1.){
                        // Debugging::out() << "Failed heuristic tests" << endl;
                        break;
                    }
                }
