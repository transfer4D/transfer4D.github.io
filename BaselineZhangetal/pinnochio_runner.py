...
class SkeletonEmbedding: 
	def __init__(self,opt,vis):
		self.pinoccio_path = "Code/Pinocchio/DemoUI/DemoUI"
	...
	def get_retargeted_motion(self,source_frame_id,num_joints,source_path,target_path,orig_data_path):
		...
		if len(parent_array) != target_attachment.shape[1] +1:
			...
			raise Error(f"Error: Attachment file:{target_attachment.shape} should have N-1 bones. Joints:{len(parent_array)}. If Joints==1 basically that means no match found")

		# Get source skel file
		skeleton_path =  os.path.join(source_path,f"{num_joints}.skel")
		...
		skeleton_motion_path = os.path.join(source_path,f"skelMotion_{num_joints}.npy")
	...
	def __call__(self,mesh_path):	
		...
		target_path = "Zhang_".join(os.path.basename(mesh_path).split('.')) + '_' + str(self.opt.anime_index)
		...
		for file in traj_files:
			...
			if num_joints >= 30: 
				num_joints = 29
			skeleton_path =  os.path.join(data_path,f"reconstructionZhang_{source_frame_id}/{num_joints}.skel")
			joint_positions,original_parent_array = self.read_skel(skeleton_path)
			...
			if os.path.isfile(os.path.join(target_path,f'{source_frame_id}_target.skel')):
				...
				retargeted_motion,faces,target_skeleton_motion = self.get_retargeted_motion(source_frame_id,num_joints,os.path.join(data_path,f"reconstructionZhang_{source_frame_id}"),target_path,data_path)
