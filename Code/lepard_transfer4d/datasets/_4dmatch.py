...
class _4DMatch(Dataset):
    ...
    def __init__(self, config, split, data_augmentation=True):
        ...
        print(f"{split} Dataset samples:{self.__len__()}")

        print([(i,x) for i,x in enumerate(self.entries) if "dinoET_act2" in x])

    def read_entries (self, split, data_root, d_slice=None, shuffle= False):
        print("Reading:",data_root,split)
    ...
    def __len__(self):
        return 1 if self.split == 'test' else len(self.entries )
    ...
    def __getitem__(self, index, debug=False):
        ...
        if self.entries[index] == "/media/srialien/Elements/AT-Datasets/4DMatch/split/split/4DMatch/dinoET_act2/cam2_0009_cam2_0021.npz":
            print(index,self.entries[index])
...
if __name__ == '__main__':
    ...
    config = "./configs/train/4dmatch.yaml"
