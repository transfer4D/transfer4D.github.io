...
def ED_warp(x, g, R, t, w):
    ...
    :return:
    """
    # print(R,x[:,None],g,w[...,None])

    # print(type(x),type(g),type(R),type(t),type(w))

    y = ( R * (x[:,None] - g) + g + t ) * w[...,None]
    y = y.sum(dim=1)
    return y

def ED_warp_normal(x,R,w):
    """ Warp normal using the embeded deformation
    https://people.inf.ethz.ch/~sumnerb/research/embdef/Sumner2007EDF.pdf
    :param x: normal (normalized)
    :param R: rotation
    :param w: weights
    :return:
    """
...
def get_deformation_graph_from_depthmap (depth_image, intrin, debug_mode=False):
    '''
    :param depth_image:
    :param intrin:
    :return:
    ...
    EROSION_MIN_NEIGHBORS = 0

    # Node sampling and edges computation
    node_coverage = 0.09  # in meters
